...
let  w = 700, h = 700, t = 1, delta = .01, padding = 10,
     n = 4, order = 5;

document.getElementById('order').addEventListener('change', changeOrder);
...
function redraw() {
    let bezier = {},
        ...
        orders = d3.range(order, order + 1),
        points = [];
    for(let i=0;i<order;i++) {
        points.push(randomPoint());
    }
    d3.select("#board").selectAll("svg").remove();

    let vis = d3.select("#board").selectAll("svg")
        .data(orders)
...
function randomColor() {
    let color = [0,1,2,3,4,5];
    return '#' + color.map(()=>Math.floor(Math.random()*10)).join('');
}
